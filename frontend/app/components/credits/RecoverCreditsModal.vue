...
const maxAmount = computed(() => props.member?.balance ?? 0)
...
async function onSubmit() {
  ...
  loading.value = true
  try {
    await creditsStore.recoverCredits({
    ...
    toast.add({
      ...
      description: t('pages.dashboard.credits.recover.success', {
        amount: form.amount,
    ...
    toast.add({
      ...
      description: t('pages.dashboard.credits.recover.error'),
      color: 'error',
    })
  }
  finally {
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-base font-semibold">
                {{ t('pages.dashboard.credits.recover.title') }}
              </h3>
            </div>
            <UButton
              variant="ghost"
        ...
        <div v-if="member" class="space-y-4">
          ...
          <div class="rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950">
            <p class="text-sm text-amber-800 dark:text-amber-200">
              {{ t('pages.dashboard.credits.recover.warning') }}
            </p>
          </div>
          ...
          <div class="space-y-4">
            <UFormField
              :label="t('pages.dashboard.credits.recover.amount')"
              name="amount"
            >
              <UInput
                ...
                :min="1"
                :max="maxAmount"
                :placeholder="t('pages.dashboard.credits.recover.amount_placeholder')"
              />
              <template #hint>
                <span class="text-sm text-gray-500">
                  {{ t('pages.dashboard.credits.recover.max_available', { max: maxAmount }) }}
                </span>
              </template>
            </UFormField>

            <UFormField
              :label="t('pages.dashboard.credits.recover.description')"
              name="description"
            >
              <UTextarea
                v-model="form.description"
                :placeholder="t('pages.dashboard.credits.recover.description_placeholder')"
                :rows="2"
              />
            </UFormField>
          </div>
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            ...
            <UButton
              ...
              @click="onSubmit"
            >
              {{ t('pages.dashboard.credits.recover.submit') }}
            </UButton>
          </div>
        </template>
      </UCard>
